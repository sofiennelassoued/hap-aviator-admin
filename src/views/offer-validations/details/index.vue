<template>
  ...
  <nav aria-label="breadcrumb" class="main-breadcrumb">
    <ol class="breadcrumb">
      ...
      <li class="breadcrumb-item"><router-link to="/users">Offer Validations</router-link></li>
...
import { getOfferValidationMetadata } from '@/domain/offer-validations';
...
onMounted(() => {
  ...
  const fn = async () => {
    try {
      loading.value = true
      payload.value = await getOfferValidationMetadata(id.value)
      loading.value = false
    } catch (error) {
      loading.value = false
      console.log(error)
