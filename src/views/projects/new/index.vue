          ...
          <div class="card">
            <div class="card-body">
              ...
              <input type="text" class="form-control" id="input-email" aria-describedby="text-email"
                placeholder="Ex: contact@happy-pizza.com / name@hap-project.com" required v-model="email">
...
import { createProjectIdentity, createProjectMetadata } from '@/domain/projects';
  ...
  const fn = async () => {
    if (imageBlob.value) {
      try {
        loading.value = true
        error.value = ""
        const { user } = await createProjectIdentity(email.value,
          password.value)
        const u = await upload(IMAGES_STORAGE_BUCKET + '/projects/' + user.uid, imageBlob.value, async (e, s) => {
          if (e) error.value
        ...
        if (u) {
          ...
          await createProjectMetadata(user.uid, metadata)
          loading.value = false
          Swal.fire({
