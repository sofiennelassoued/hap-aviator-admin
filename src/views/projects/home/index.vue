<template>
              ...
              <tr v-for="item in items" :key="item.id">
                ...
                <td>
                  <router-link type="button" class="btn btn-sm btn-outline-primary mx-2"
                    :to="'/projects/' + item.id">View</router-link>
                  <router-link type="button" class="btn btn-sm btn-outline-secondary"
                    :to="'/offers/new?project=' + item.id">Create offer</router-link>
                </td>
              </tr>
            </tbody>
          </table>
  ...
  <div class="vh-100 d-flex justify-content-center align-items-center" v-if="!loading && items.length === 0">
    <div class="text-center">
      <p>No items</p>
      <router-link type="button" class="btn btn-sm btn-outline-primary mx-2"
        :to="'/projects/new'">Create</router-link>
    </div>
  </div>
...
onMounted(() => {
  const fn = async () => {
    try {
      loading.value = true
      allItems.value = await getProjects()
